...
# 使用真实 API 替代动态页面
urls = [
    'https://api.vvhan.com/api/CloudFlare',  # JSON 格式，替代 https://cf.vvhan.com/
    'https://ip.164746.xyz',                 # HTML
    'https://github.com/hubbylei/bestcf/raw/refs/heads/main/bestcf.txt',  # 纯文本
    'https://addressesapi.090227.xyz/CloudFlareYes'  # JSON（嵌套表格）
]
...
for url in urls:
    ...
    # JSON 格式
    if 'application/json' in content_type or 'api.vvhan.com' in url or url.endswith('.json'):
        try:
            data = response.json()
            # API 格式是：{ "success": true, "ip": [ "IP1", "IP2", ... ] }
            ip_data = data.get('ip') or data.get('data')
